...
@web_app.route('/')
def home():
    return "Bot Server is Alive 24/7!"
...
# ================= GLOBAL USER STATES =================
user_action_state = {}   # {user_id: {"action": "...", "input_val": "...", "deposit_id": ...}}
admin_action_state = {}  # {admin_id: {"action": "...", "input_val": "...", ...}}
authenticated_admins = set()

# ================= USER KEYPAD GENERATOR =================
def generate_user_keypad(amount_str):
    text = (
        ...
        "Use on-screen keypad or type directly in chat.\n"
...
async def user_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user and user[4] == 1:
        ban_text = "⛔ **Account Banned:** You have been restricted from accessing this bot."
        if query:
            await query.edit_message_text(ban_text, parse_mode="Markdown")
        else:
    ...
    if get_setting("maintenance") == "ON" and tg_user.id != ADMIN_ID:
        maint_text = "🛠 **SYSTEM UNDER MAINTENANCE**\n\nWe are currently upgrading our servers. Please check back shortly!"
        if query:
            await query.edit_message_text(maint_text, parse_mode="Markdown")
        else:
            await update.message.reply_text(maint_text, parse_mode="Markdown")
        return
    ...
    if action == "WAIT_UTR":
        ...
        try:
            ...
            admin_markup = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("✅ Approve", callback_data=f"dep_app_{deposit_id}"),
                    InlineKeyboardButton("❌ Reject", callback_data=f"dep_rej_{deposit_id}")
                ]
            ])
            await admin_bot.bot.send_message(ADMIN_ID, notif_text, reply_markup=admin_markup, parse_mode="Markdown")
...
async def user_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "user_deposit_init":
        ...
        if pending:
            pend_text = (
                ...
                f"You already have a pending request of `₹{pending[1]:.2f}` (ID: #{pending[0]}).\n"
                "Please wait for admin approval or cancel it to make a new deposit."
            )
    ...
    if data.startswith("user_cancel_dep_"):
        ...
        conn.commit()
        conn.close()
        await query.edit_message_text("✅ Your pending deposit request was cancelled.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Return", callback_data="main_menu_user")]]))
        return

    # Keypad handler for deposit
    ...
    # Purchase Key Flow with Auto-Refund Protection
    if data.startswith("buy_"):
        ...
        await query.edit_message_text(f"⏳ **Verifying order with provider API for `{duration}`...**", parse_mode="Markdown")
        ...
        try:
            res = requests.post(API_ENDPOINT, data=payload, headers=headers, timeout=25)
            try:
                data_resp = res.json()
            except Exception:
            ...
            if license_key or data_resp.get("status") == "success":
                ...
                # 3. AUTO-REFUND IF API FAILS / MAINTENANCE
                conn = get_db()
                c = conn.cursor()
                ...
                err_msg = data_resp.get("msg") or data_resp.get("message") or "Provider Server Under Maintenance"
                fail_text = f"❌ **API Server Error:** `{err_msg}`\n\n💰 **₹{cost:.2f} was refunded** back to your wallet instantly."
...
async def user_proceed_payment(update: Update, context: ContextTypes.DEFAULT_TYPE, is_callback=True):
    ...
    user_action_state[user_id] = {"action": "WAIT_UTR", "deposit_id": deposit_id, "amount": amount}
    ...
    caption = (
        ...
        f"🌐 **UPI ID:** `{upi_id}`\n\n"
        "1️⃣ Scan & Pay the exact amount above.\n"
        "2️⃣ Copy the **12-digit UTR / Ref No** after paying.\n"
        "3️⃣ **Type & Send the UTR / Ref ID directly in chat.**"
    )
    markup = InlineKeyboardMarkup([[InlineKeyboardButton("🚫 Cancel Transaction", callback_data=f"user_cancel_dep_{deposit_id}")]])

    if is_callback:
        await update.callback_query.message.delete()
        await context.bot.send_photo(chat_id=user_id, photo=qr_api_url, caption=caption, reply_markup=markup, parse_mode="Markdown")
    else:
        await context.bot.send_photo(chat_id=user_id, photo=qr_api_url, caption=caption, reply_markup=markup, parse_mode="Markdown")
...
async def send_admin_panel(chat_id, context: ContextTypes.DEFAULT_TYPE, message_id=None):
    ...
    maint_btn_text = "🛠 Maintenance: [ON] (Click to Turn OFF)" if maint_status == "ON" else "✅ Maintenance: [OFF] (Click to Turn ON)"
    ...
    if action == "ADD_BAL":
        ...
        if row:
            ...
            try:
                ...
                await main_bot.bot.send_message(target_id, f"🎉 **Wallet Credited!**\nAdmin added `₹{amount:.2f}` to your account.\n💰 Current Balance: `₹{new_bal:.2f}`", parse_mode="Markdown")
            except Exception:
                pass
        else:
...
async def admin_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Approvals for Deposit Requests
    ...
    if data == "admin_main_menu":
        ...
        admin_action_state[user_id] = {}
        await query.edit_message_text("🔒 Logged out successfully. Send /start to login again.")
        ...
        await query.edit_message_text("👥 **User Management:**\nSearch by ID/Username or select below:", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
...
def main():
    ...
    print("🚀 All systems operational: Main Bot & Admin Bot are running...")
    run_main_bot()
