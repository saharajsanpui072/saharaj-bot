...
def get_setting(key):
    settings = fb_get("settings")
    return settings.get(key, "") if settings else ""
...
# ================= FLASK WEB SERVER & HTML ADMIN PANEL =================
...
<head>
    ...
    <title>Saharaj Bot Cloud Admin</title>
...
async def user_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "user_deposit_init":
        ...
        pending = next((d for d in deposits.values() if d and d.get("user_id") == user_id and d.get("status") == "PENDING"), None)
...
async def handle_admin_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_action == "SEARCH_USER":
        ...
        for u in users.values():
            if u and (str(u.get("user_id")) == query_str or query_str in str(u.get("username", "")).lower() or query_str in str(u.get("full_name", "")).lower()):
                user_info = u
                break
...
async def admin_callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("dep_app_"):
        ...
        if dep and dep.get("status") == "PENDING":
            ...
            cur_bal = float(user.get("balance", 0.0)) if user else 0.0
...
async def run_unified_system():
    ...
    print("🚀 Firebase Cloud Bot & Web Admin Panel Running Live!")
